...
S.desired_torques = 0.01 * sind(desired_angles);
...
optimal_params = fmincon(@objective_function, init_params, [], [], [], [], [phi_moment_arm_lb, moment_arm_location_lb, L_rod_lb], [phi_moment_arm_ub, moment_arm_location_ub, L_rod_ub]);

disp(['Optimal Moment Arm Angle: ', num2str(optimal_params(1))]);
P.phi_moment_arm = deg2rad(optimal_params(1));

disp(['Optimal Moment Arm Location: ', num2str(optimal_params(2))]);
P.moment_arm_location = optimal_params(2);

disp(['Optimal Rod Length: ', num2str(optimal_params(3))]);
...
videofile = fullfile([pwd '\MATLAB Figures'], 'ExoNET_Rod_v2_Movie.mp4'); % file save path
...
saveas(figure(1), fullfile([pwd '\MATLAB Figures'],'ExoNET_Rod_v2_Torque.png')); % save to computer
...
saveas(figure(2), fullfile([pwd '\MATLAB Figures'],'ExoNET_Rod_v2_Supplemental.png')); % save to computer
